...
- [ ] **(Sprint 5)** HR is manager-less in seed data, so an HR account's own manager-less leave request lands in the same `GetPendingRequestsForHrFallbackAsync()` queue that HR administers — `ApprovalService.DecideAsync`'s eligibility check (`ManagerId is null && deciderIsHr`) doesn't exclude `deciderId == request.EmployeeId`, so HR could in principle approve/reject their own request. Accepted as a reasonable, intentional consequence of the sprint plan's resolution of PRD open question #1 for a small-company MVP (a sole HR account has no other eligible decider anyway, so a same-person guard alone would just convert "HR self-approves" into "HR's request is undecidable"). Given the PRD's "0 discrepancies" goal and auditability NFR, track for Sprint 7 or a later HR-admin sprint: either add a same-person guard with a defined fallback (e.g. a second HR/owner account decides, or the request is flagged for manual reconciliation), or explicitly document in the PRD/ops runbook that a sole HR account's own leave is self-administered by policy.
